...
const create = async (
    ...
    if (yearBandWasFormed.trim().length === 0)
      throw 'yearBandWasFormed cannot be an empty string or string with just spaces';
    yearBandWasFormed = yearBandWasFormed.trim();
    if (yearBandWasFormed < 1900) throw 'yearBandWasFormed should be from 1900-2023';
    if (yearBandWasFormed > 2023) throw 'yearBandWasFormed should be from 1900-2023';
  ...
  async function main() {
      const pinkFloyd = await create("Pink Floyd", ["Progressive Rock", "Psychedelic rock", "Classic Rock"], "http://www.pinkfloyd.com", "EMI", ["Roger Waters", "David Gilmour", "Nick Mason", "Richard Wright", "Sid Barrett" ], 1965);
